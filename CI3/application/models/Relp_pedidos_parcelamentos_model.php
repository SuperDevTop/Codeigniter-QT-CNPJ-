...
class Relp_pedidos_parcelamentos_model extends CI_Model {
    ...
    public function clear($cnpj, $banco){
        return $this->db->delete($banco.'.dtb_ecac_relp_pedidos_parcelamentos', "cnpj = '{$cnpj}'");
    }
    ...
    public function insert($dados, $banco){
        ...
        $this->db->insert($banco.'.dtb_ecac_relp_pedidos_parcelamentos', $this);
        return $this->db->insert_id();
    }

    public function verifica_se_existe($cnpj, $banco, $numero){
        $this->db->select('COUNT(distinct(dtb_ecac_relp_pedidos_parcelamentos.id)) AS qtd, dtb_ecac_relp_pedidos_parcelamentos.id as id');
        $this->db->where('cnpj', $cnpj);
        $this->db->where('numero', $numero);
        return $this->db->get($banco.'.dtb_ecac_relp_pedidos_parcelamentos')->row();
    }
    ...
    public function update($cnpj, $banco, $numero,  $situacao){
        ...
        $this->db->where('cnpj', $cnpj);
        $this->db->where('numero', $numero);
        $this->db->update($banco.'.dtb_ecac_relp_pedidos_parcelamentos');
    }
