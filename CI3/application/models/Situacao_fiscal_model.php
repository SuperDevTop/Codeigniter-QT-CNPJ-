...
class Situacao_fiscal_model extends CI_Model {
	...
	public function verifica_se_existe($cnpj_data, $banco){
		$this->db->select('COUNT(distinct(dtb_situacao_fiscal.id)) AS qtd');
		$this->db->where('cnpj_data', $cnpj_data);
		return $this->db->get($banco.'.dtb_situacao_fiscal')->row();
	}
	...
	public function insert($possui_pendencia, $data_pdf, $caminho_download, $cnpj_data, $banco){
		...
		$this->db->insert($banco.'.dtb_situacao_fiscal', $this);
	...
	public function update($possui_pendencia, $data_pdf, $caminho_download, $cnpj_data, $banco){
		...
		if ($this->db->update($banco.'.dtb_situacao_fiscal', $dados, "cnpj_data=".$cnpj_data)){
			return TRUE;
		} else {
			return FALSE;
		}
