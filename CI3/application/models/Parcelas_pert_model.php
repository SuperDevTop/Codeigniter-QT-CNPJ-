...
class Parcelas_pert_model extends CI_Model {
    ...
    public function get_parcela_sem_emitir($banco, $cnpj){
        ...
        return $this->db->get($banco.'.dtb_pert_parcelas_emitidas')->row();
    }
	...
	public function insert($cnpj, $banco, $dados)
	{
		...
		$this->cnpj = $cnpj;
		$this->db->insert($banco.'.dtb_pert_parcelas_emitidas', $this);
	}
    ...
    public function update($cnpj, $banco, $dados)
    {
        ...
        $this->db->where('cnpj', $cnpj);
        $this->db->update($banco.'.dtb_pert_parcelas_emitidas');
    }

    public function verifica_se_existe($banco, $cnpj, $data_parcela ){
        $this->db->select('COUNT(distinct(dtb_pert_parcelas_emitidas.data_parcela)) AS qtd, path_download_parcela');
        $this->db->where('cnpj', $cnpj);
