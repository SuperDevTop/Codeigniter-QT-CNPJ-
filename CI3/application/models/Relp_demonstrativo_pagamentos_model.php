...
class Relp_demonstrativo_pagamentos_model extends CI_Model {
    ...
    public function clear($cnpj, $banco){
        return $this->db->delete($banco.'.dtb_ecac_relp_demonstrativo_pagamentos', "cnpj = '{$cnpj}'");
    }
    ...
    public function insert($dados, $banco){
        ...
        $this->db->insert($banco.'.dtb_ecac_relp_demonstrativo_pagamentos', $this);
        return $this->db->insert_id();
    }

    public function verifica_se_existe($cnpj, $banco, $id_parcelamento, $mes_parcela){
        $this->db->select('COUNT(distinct(dtb_ecac_relp_demonstrativo_pagamentos.id)) AS qtd');
        $this->db->where('cnpj', $cnpj);
        $this->db->where('id_parcelamento', $id_parcelamento);
        $this->db->where('mes_parcela', $mes_parcela);
        return $this->db->get($banco.'.dtb_ecac_relp_demonstrativo_pagamentos')->row();
    }
