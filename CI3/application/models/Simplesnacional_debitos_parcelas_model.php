...
class Simplesnacional_debitos_parcelas_model extends CI_Model {
    ...
    public function clear($cnpj, $banco){
        return $this->db->delete($banco.'.dtb_ecac_simplesnacional_relacao_debitos_parcelas', "cnpj = '{$cnpj}'");
    }
    ...
    public function insert($dados, $banco){
        ...
        $this->db->insert($banco.'.dtb_ecac_simplesnacional_relacao_debitos_parcelas', $this);
        return $this->db->insert_id();
    }

    public function verifica_se_existe($cnpj, $banco, $id_parcelamento){
        $this->db->select('COUNT(distinct(dtb_ecac_simplesnacional_relacao_debitos_parcelas.id)) AS qtd');
        $this->db->where('cnpj', $cnpj);
        $this->db->where('id_parcelamento', $id_parcelamento);
        return $this->db->get($banco.'.dtb_ecac_simplesnacional_relacao_debitos_parcelas')->row();
    }
}
