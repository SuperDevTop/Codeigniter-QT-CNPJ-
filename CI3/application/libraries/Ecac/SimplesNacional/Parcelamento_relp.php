...
class Parcelamento_relp extends Ecac
{
    ...
    {
        $page = $this->exec('https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/Default.aspx', array(''), '', false, $this->cookie_complemento);
        ...
        $table = $html->find('table[id=ctl00_contentPlaceH_ucListaParcelamentos_gdv]', 0);
        if ($table) {
            $trs = $table->find('tr');
            $posicao  = 0;
            foreach ($trs as $tr) {
                if ($posicao > 0) {
                    ...
                    if ($status == 'Em parcelamento') {
                        $__EVENTTARGET = 'ctl00$contentPlaceH$ucListaParcelamentos$gdv';
    ...
    private function obter_relacao_debitos_parcelas($page, $numero_parcelamento)
    {
        ...
        $html = new Simple_html_dom();
        $html->load($page);
        $table = $html->find("table[id=ctl00_contentPlaceH_ucListaDebitos_gdv]", 0);
        $trs = $table->find('tr');
        $i = 0;
    ...
    private function post($post_fields)
    {
        ...
        $url = "https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/Default.aspx";
        ...
        $headers = array(
            ...
            "Sec-Fetch-User: ?1",
            "Sec-Fetch-Dest: iframe",
            "Referer: https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/Default.aspx",
            "Accept-Language: pt-BR,pt;q=0.9",
        );
    ...
    {
        $url = "https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/ConsultaPedido.aspx";
    ...
    {
        $url = "https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/ConsultaPedidoDetalhes.aspx";
        $headers = array(
            "Connection: keep-alive",
            "Cache-Control: max-age=0",
            'sec-ch-ua: " Not A;Brand";v="99", "Chromium";v="96", "Google Chrome";v="96"',
            ...
            "Sec-Fetch-User: ?1",
            "Sec-Fetch-Dest: iframe",
            "Referer: https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/ConsultaPedidoDetalhes.aspx",
            "Accept-Language: pt-BR,pt;q=0.9",
        );
    ...
    function obter_simples_nacional_emissao_parcela()
    {
        ...
        //     return;
        $page = $this->exec('https://sinac.cav.receita.fazenda.gov.br/simplesnacional/aplicacoes/ATSPO/relpsn.app/Default.aspx');
        $html = new Simple_html_dom();
        $html->load($page);
        ...
        $div_principal = $html->find('table[id=ctl00_contentPlaceH_gdv]', 0);
    ...
    function download_parcela_simplesnacional($data_parcela)
    {
        ...
        while ($tentativa) {
            ...
            $url = "https://sinac.cav.receita.fazenda.gov.br/simplesnacional/Aplicacoes/ATSPO/relpsn.app/ReemitirDas.aspx";
            $headers = array(
                "Connection: keep-alive",
                "Cache-Control: max-age=0",
                'sec-ch-ua: " Not A;Brand";v="99", "Chromium";v="96", "Google Chrome";v="96"',
            ...
            $div_principal = $html->find('table[id=ctl00_contentPlaceH_pnlParcelas]', 0);
            $retorno = '';
            if ($div_principal) {
                $linhas = $div_principal->find('tr');
                if (count($linhas) == 0) {
                ...
                foreach ($linhas as $linha) {
                    if ($data_parcela ==  trim($linha->find('td', 0)->plaintext)) {
                        $caminho_local = $this->caminho_da_pasta_pdfs . "/DAS-Simples-Nacional-Relp-{$data_atual}.pdf";
