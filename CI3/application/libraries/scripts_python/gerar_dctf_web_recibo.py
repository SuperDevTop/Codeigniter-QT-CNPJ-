...
def gera_recibo(session, soup, new_cookie):
    ...
    if table:
        ...
        for id in range(1, numero_linhas):
            ...
            if ID_DECLARACAO == id_declaracoes[0][2:] and ID_CONTROLE == id_declaracoes[1][2:]:
                data = {
                    '__EVENTTARGET': 'ctl00$cphConteudo$tabelaListagemDctf$GridViewDctfs$ctl'+str(id+1).zfill(2)+'$lkbVisualizarRecibo',
                    '__EVENTARGUMENT': '',
...
def executar():
    with requests.Session() as session:
        ...
        caminho_pdf = gera_recibo(session, soup, new_cookie)
