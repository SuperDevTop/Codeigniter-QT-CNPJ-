...
// Primeiramente fazemos login e pegamos o cookie de sessao
    ...
    function baixar_pdf_cadin()
    {
        ...
        curl_setopt($this->curl, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($this->curl, CURLOPT_COOKIEJAR, $this->cookie_path);
        $fp = fopen ($this->caminho_da_pasta_pdfs."/cadin-{$this->numero_documento_procuracao}.pdf", 'w+');
        curl_setopt($this->curl, CURLOPT_FILE, $fp);
        $response = curl_exec($this->curl);
        if(curl_errno($this->curl))
        {
        ...
        return $this->caminho_da_pasta_pdfs."/cadin-{$this->numero_documento_procuracao}.pdf";
    }
    ...
    public function get_dctf_declaracao_individual($periodo_busca){
        ...
        if($tbDeclaracoes){
		    ...
		    Require_once  '/var/www/html/ba/vendor/autoload.php';
		    $mpdf = new \Mpdf\Mpdf();
                    $mpdf->WriteHTML($html);
                    $mpdf->Output($this->caminho_da_pasta_pdfs."{$this->obter_numero_documento()}-dctf-declaracao-{$UltimoSel}.pdf",'F');
                }catch (Exception $e){
                ...
                $caminho_pdf = str_replace("/var/www/html/", "https://servicosveri.com.br/", $this->caminho_da_pasta_pdfs)."{$this->obter_numero_documento()}-dctf-declaracao-{$UltimoSel}.pdf";
...
//                return false;
    ...
    public function get_das($cnpj, $ano=''){
        ...
        if ($ano == '')
            $ano = date("Y");
        $this->setar_numero_documento($cnpj);
        $response = shell_exec("python {$this->path_script_simples_nacional} \"{$this->cert_key}\" \"{$this->private_key}\" \"{$cnpj}\" \"{$this->caminho_da_pasta_pdfs}\" \"{$ano}\"");
    ...
    public function get_das_debitos($cnpj){
        ...
        $this->setar_numero_documento($cnpj);
        $response = shell_exec("python {$this->path_script_simples_nacional_debitos} \"{$this->cert_key}\" \"{$this->private_key}\" \"{$cnpj}\"");
        return json_decode( preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $response), true);
    }

    public function gerar_das($competencia, $cnpj=''){
        $this->encerrar_conection();
        ...
        curl_setopt($ch, CURLOPT_URL, "http://0.0.0.0:5000/gerar-das");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_TIMEOUT, 10000);
