...
//		o pdf sem fazer isso, o pdf vem vazio.
	...
	function obter_pdf($is_post = false, $url)
	{
		...
		curl_setopt($this->curl, CURLOPT_HTTPHEADER, $headers);
		curl_setopt($this->curl, CURLOPT_COOKIEJAR, 'cookie.txt');
		$fp = fopen ($this->caminho_da_pasta_pdfs."/situação-fiscal-{$this->numero_documento}.pdf", 'w+');
		...
		return $this->caminho_da_pasta_pdfs."/situação-fiscal-{$this->numero_documento}.pdf";
	}
...
// Primeiramente fazemos login e pegamos o cookie de sessao
	...
	public function gerar_das($competencia){
        $post = array(
